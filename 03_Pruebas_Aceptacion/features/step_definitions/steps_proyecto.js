...
Then('genero un PDF de {string}', async function (escenario) {
    if (!pdfDoc) {
      pdfDoc = await PDFDocument.create();
  }
  // Tomar una captura de pantalla con Selenium
  const screenshotPath = path.join(__dirname, `screenshot_proyect${pageIndex}.png`);
...
AfterAll(async function () {
    ...
    if (pdfDoc) {
        ...
        fs.writeFileSync(path.join(__dirname, 'documento_Pruebas_Seccion_detalle_Proyestos.pdf'), pdfBytes);
...
Then('cerrar session proyectos', async function () {
...
Then('Carga Imagenes en tiempo estimado Pr', async function () {
