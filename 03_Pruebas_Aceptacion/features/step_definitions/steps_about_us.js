...
Then('genero un PDF de para {string}', async function (escenario) {
    if (!pdfDoc) {
      pdfDoc = await PDFDocument.create();
  }
  // Tomar una captura de pantalla con Selenium
  const screenshotPath = path.join(__dirname, `screenshot_about${pageIndex}.png`);
...
AfterAll(async function () {
    ...
    if (pdfDoc) {
        ...
        fs.writeFileSync(path.join(__dirname, 'documento_Pruebas_Seccion_Detalle_AboutUs.pdf'), pdfBytes);
...
Then('cerrar session about us', async function () {
...
Then('Carga Imagenes en tiempo estimado A', async function () {
